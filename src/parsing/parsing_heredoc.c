...
/*   Updated: 2023/12/20 00:57:18 by keschouf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	Pre-parsing function for commands with quotes. Used mainly for the
	heredoc, as single pairs of quotes must not be interpreted as a delimiter
	by the heredoc. Quote parsing for other commands is done later.
	...
	The heredoc_flag ensures we are not inside the heredoc, in which case 
	changes to the variables should not affect the environment. Otherwise, 
...
void	count_quotes_heredoc(char *cmd, t_data *data, int *j)
{
	...
	open = 0;
	data->c = get_first_quote(&cmd[data->i]);
	while (cmd[(data->i)])
	{
		...
		if (!data->heredoc_flag
			|| (data->heredoc_flag && cmd[data->i] != data->c))
			*j += 1;
...
/*
	Same logic as count_quotes_heredoc() : the command is parsed while 
...
void	args_parsing(char *cmd, t_data *data, t_struct *itr, int j)
{
	...
	while (data->pos <= data->i)
	{
		...
		if (!data->heredoc_flag
			|| (data->heredoc_flag && cmd[data->pos] != data->c))
